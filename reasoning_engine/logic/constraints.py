...
def _dry_run_constraint_violation(claim: str, texts: List[str]) -> bool:
    ...
    constraint_keywords = [
        "first time", "never", "had not", "has not", "have not",
        "never before", "for the first time", "unfamiliar",
        "unknown", "unseen", "unheard", "never seen", "never heard"
    ]
    
    # Check if claim suggests repeated/ongoing activity
    claim_suggests_repetition = any(word in claim_lower for word in [
        "years", "often", "frequently", "regularly", "repeatedly",
        "many times", "multiple", "several", "always", "often"
    ])
    ...
    for text in texts:
        ...
        violation_signals = sum(1 for keyword in constraint_keywords if keyword in text_lower)
        ...
        if any(strong_signal in text_lower for strong_signal in [
            "first time", "never before", "for the first time"
        ]):
            return True
