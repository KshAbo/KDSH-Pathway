...
def _dry_run_contradiction(claim: str, text: str) -> bool:
    ...
    contradiction_keywords = [
        "never", "not", "no", "didn't", "doesn't", "wasn't", "weren't",
        "cannot", "couldn't", "wouldn't", "shouldn't", "hadn't",
        "impossible", "false", "incorrect", "wrong"
    ]
    
    # Strong temporal contradiction signals
    temporal_negation = ["never", "first time", "never before", "for the first time"]
    temporal_repetition = ["years", "often", "frequently", "regularly", "repeatedly", 
                          "many times", "multiple", "several", "always"]
    ...
    if any(strong_neg in text_lower for strong_neg in ["never", "impossible", "false"]):
        ...
        action_words = {"travel", "traveling", "go", "went", "visit", "visited", 
                       "see", "saw", "experience", "experienced", "leave", "left"}
