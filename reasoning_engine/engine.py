...
def generate_evidence_rationale(
    ...
    Only generates explanations for CONTRADICT, INCOMPATIBLE, and optionally SUPPORT chunks.
    ...
    # Optional: Add SUPPORT chunks if decision is 1 (positive case)
    # Only add a few key supporting chunks as heuristic
    if decision == 1 and len(rationale) == 0:
        # Add first chunk as supporting evidence (heuristic)
        for analysis in contradiction_analysis[:1]:  # Just first one
            if analysis["relation"] == "NEUTRAL":
                rationale.append(
                    {
                        "chunk_id": analysis["chunk_id"],
                        "excerpt": analysis["excerpt"],
